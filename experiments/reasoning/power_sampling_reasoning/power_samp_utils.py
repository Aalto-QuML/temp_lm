...
def format_prompt(question, model, tokenizer, cot=True):
    # Normalize model string for case-insensitive substring checks
    model_lower = model.lower() if isinstance(model, str) else ""

    # Helper to build the simple prompt (used by many models)
    def _simple_prompt():
        s = PROMPT + question
        s += COT if cot else BASE
        return s

    # Prefer more specific identifiers first
    if "qwen_math_grpo" in model_lower or "qwen_math_grpo" == model_lower:
        content_str = _simple_prompt()
    ...
    elif "qwen_math" in model_lower:
        format_str = _simple_prompt()

    elif "qwen" in model_lower:
        format_str = _simple_prompt()

    elif "phi_grpo" in model_lower or "phi_grpo" == model_lower:
        content_str = _simple_prompt()
        answer_context = [{"role": "user", "content": content_str}]
        format_str = tokenizer.apply_chat_template(
            answer_context, tokenize=False, add_generation_prompt=True
    ...
    elif "phi" in model_lower or "phi35" in model_lower:
        content_str = _simple_prompt()
        answer_context = [{"role": "user", "content": content_str}]
        format_str = tokenizer.apply_chat_template(
            answer_context, tokenize=False, add_generation_prompt=True
        ...
        # Fallback: build a simple prompt so format_str is always defined
        format_str = _simple_prompt()
