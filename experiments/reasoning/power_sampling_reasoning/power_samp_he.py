...
from experiments.reasoning.power_sampling_reasoning.grader_utils.parse_utils import (
    parse_answer,
)
from experiments.reasoning.power_sampling_reasoning.constants import *
from experiments.reasoning.power_sampling_reasoning.power_samp_utils import *
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--temperature", action="store", default=0.5, type=float, dest="temperature"
    )
    parser.add_argument("--dataset", action="store", default="HUMANEVAL", type=str)
    parser.add_argument("--cot", action="store", type=bool, default=True)
    parser.add_argument(
        "--type", action="store", type=str, default="chat", choices=["chat"]
    )
    ...
    if model == "qwen":
        ...
        model_str = f"/m/cs/scratch/temperature_diffusion/models/{model}"

    use_remote_code = not model_str.startswith(
        "/m/cs/scratch/temperature_diffusion/models/"
    )
    ...
    if dataset_name == "HUMANEVAL":
        ...
        candidate_paths = [
            os.path.join(
                script_dir, "data", "HumanEval.jsonl", "human-eval-v2-20210705.jsonl"
            ),
            os.path.join(
                repo_root, "data", "HumanEval.jsonl", "human-eval-v2-20210705.jsonl"
            ),
            "data/HumanEval.jsonl/human-eval-v2-20210705.jsonl",
        ]

        json_file = next((p for p in candidate_paths if os.path.exists(p)), None)
        if json_file is not None:
            with open(json_file, "r", encoding="utf-8") as f:
                dataset = [json.loads(line) for line in f if line.strip()]
            print(f"Loaded HumanEval from local file: {json_file}")
        else:
            print(
                "HumanEval jsonl not found locally; loading from Hugging Face dataset "
                "openai/openai_humaneval instead."
            )
    ...
    config = transformers.AutoConfig.from_pretrained(
        model_str,
        trust_remote_code=use_remote_code,  # local_files_only=True
    )
    ...
    for problem, data in tqdm(
        enumerate(dataset[start:end]), desc="Benchmark on HumanEval"
    ):
        prompt = data["prompt"]
    ...
    df.to_csv(
        os.path.join(
            ...
            # + str(mcmc_steps)
            # + "_"
            + str(temp) + "_" + str(args.batch_idx) + "_" + str(args.seed) + ".csv",
        ),
        index=False,
    )
