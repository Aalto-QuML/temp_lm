...
from experiments.gpt2_lhts.model_gpt2 import GPT2Head
...
def process_openwebtext(example, tokenizer, max_length):
    ...
    encodings_dict = tokenizer(
        txts, truncation=True, max_length=max_length, padding="max_length"
    )
...
class Runner:
    def __init__(self, cfg):
        ...
        with torch.no_grad():
            self.gpt2_tokenizer = GPT2Tokenizer.from_pretrained(
                self.cfg.gpt_name, pad_token="<|endoftext|>"
            )
        ...
        self.model_wrap = GPT2Head(
            _len=self.cfg.context,
            model=self.model,
            scorer=self.scorer,
        ...
        dataset = dataset.select(range(0, 30000))
        self.train_loader = DataLoader(dataset, batch_size=cfg.batch, shuffle=True)

        if self.distributed:
            self.model_wrap = torch.nn.parallel.DistributedDataParallel(
                self.model_wrap,
        ...
        if self.distributed:
            ...
            self.train_loader = DataLoader(
                dataset, batch_size=cfg.batch, sampler=self.train_sampler
            )
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p
                    for n, p in self.model_wrap.named_parameters()
                    if not any(nd in n for nd in no_decay)
                    and not n.startswith("scorer")
                ],
                "weight_decay": 0.001,
            },
            {
                "params": [
                    p
                    for n, p in self.model_wrap.named_parameters()
                    if any(nd in n for nd in no_decay) and not n.startswith("scorer")
                ],
                "weight_decay": 0.0,
            },
        ]
    ...
    def train(self):
        ...
        while self.epoch < self.cfg.n_epochs:
            ...
            if self.cfg.data_source == "data":
                data_source_fn = lambda ex, _: process_openwebtext(
                    ex, self.gpt2_tokenizer, self.cfg.context
                )
            elif self.cfg.data_source == "sample":
                data_source_fn = lambda _: self.sample_model(
                    batch=self.cfg.batch, length=self.cfg.context
                )
            ...
            for it, ex in enumerate(dataloader):
                ...
                lossT, info = model.temperature_horizon_loss(
                    input_ids, attn_mask, self.cfg.horizon_loss, debug
                )
                lossN = info["loss"]
                lossK = info["kll"]
                lossT.backward()
                ...
                if accum >= self.cfg.batch_step // self.cfg.world_size:
                    if self.master_node:
                        ...
                        print(
                            "Iter %u out of %u, lossN: %.2f, lossT: %.2f, lr: %f"
                            % (it, len(dataloader), lossN, lossT, last_lr[0])
                        )
                        wandb.log(
                            {
                                "iter": (it + 1 + len(dataloader) * self.epoch)
                                * self.cfg.batch,
                                "batch lossN": lossN,
                                "batch lossT": lossT,
                                "clamp ratio": info["clamp_entries"]
                                / info["total_entries"],
                            }
                        )
                        accum = 0
            ...
            if self.master_node:
                ...
                if self.epoch > 0 and self.epoch % self.save_every == 0:
                    torch.save(
                        states,
                        os.path.join(
                            self.cfg.model_dir, "checkpoint_{}.pth".format(self.epoch)
                        ),
                    )
            ...
            if self.epoch % self.eval_every == 0:
                ...
                if self.master_node:
                    ...
                    print(
                        "Epoch %u out of %u, train lossN: %.2f, train lossT: %.2f, train lossK: %.2f"
                        % (
                            self.epoch,
                            self.cfg.n_epochs,
                            metric_tensor[0],
    ...
    def sample_model(self, batch, length):
        with torch.no_grad():
            ...
            for i, sample_output in enumerate(sample_outputs):
                txt = self.gpt2_tokenizer.decode(
                    sample_output, skip_special_tokens=True
                )
                encodings_dict = self.gpt2_tokenizer(
                    txt + "<|endoftext|>",
                    truncation=True,
                    max_length=length,
                    padding="max_length",
    ...
    def _score_samples(self, batch, c_myopic, length, debug=False):
        ...
        with torch.no_grad():
            ...
            if self.master_node:
                if debug:
                    self._text_tab = wandb.Table(
                        columns=["epoch", "i", "text", "score"]
                    )

                    for i in range(len(sample_txts)):
                        self._text_tab.add_data(
                            self.epoch, i, sample_txts[i], sample_scores[i]
                        )
                    wandb.log(
                        {
                            "text": self._text_tab,
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            while tot_batch > 0:
                ...
                samp_score = self._score_samples(
                    batch=cur_batch, c_myopic=1.0, length=length, debug=debug
                )
        ...
        with torch.no_grad():
            ...
            if self.master_node:
                print(
                    "test count %u sample_score: %.4f" % (outer_batch, metric_tensor[0])
                )
